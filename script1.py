...
def date_if(prefer_sit, driver):
    ...
    end_time_str = "17:00"
    adjusted_end_time = (datetime.strptime(end_time_str, "%H:%M") - timedelta(hours=1)).strftime("%H:%M")
    if times_roll and len(times_roll) > 0:
        ...
        if all(time in full_day_times for time in get_time_range(full_day_times, "11:00", adjusted_end_time)):
            day_type = 3
...
def choose_it(driver, sit_avilable, idx, reading_room, day_type, max_attempts=500):
    ...
    bool: 预约是否成功
    """
    dir_time = {3: [['11:00', '17:00'], ['11:00', '17:00']],
        ...
        try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if "预约成功" in result_text:
                                    ...
                                    print(get_beijing_time().strftime("%Y-%m-%d %H:%M:%S"))
                                    # 预约成功时再次截图
                                    timestamp = time.strftime("%Y%m%d_%H%M%S")
            ...
            if attempt < max_attempts:
                ...
                try:
                    ...
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located(
                            (By.CSS_SELECTOR, ".el-select__caret.el-input__icon.el-icon-arrow-up"))
                    )
                    choose_sit(driver, reading_room)
                except:
                    print("刷新页面失败")
                    break
            ...
            if reserve_box:
                ...
                if times_roll:
                    ...
                    if all(time in time_texts for time in
                           get_time_range(full_day_times, "11:00", adjusted_end_time)):
                        print(f"找到全天可预约座位: {i}")
                        found_full_day = True
                        day_type = 3
...
def main():
    ...
    sit_avilable, day_type = None, None
    users = {"自定义": [account_password4, "四楼北自习室", "24"]}
    user = "自定义"
